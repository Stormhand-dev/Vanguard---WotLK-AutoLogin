...
if AUTOLOGIN_CHAR_ORDER == nil then AUTOLOGIN_CHAR_ORDER = {} end
...
function AccountLogin_OnLoad(self)
	...
	-- Reposicionar Cinematics e Options logo acima do Quit
	AccountLoginCinematicsButton:ClearAllPoints();
	AccountLoginCinematicsButton:SetPoint("BOTTOM", AccountLoginExitButton, "TOP", 0, 5);
	OptionsButton:ClearAllPoints();
	OptionsButton:SetPoint("BOTTOM", AccountLoginCinematicsButton, "TOP", 0, -2);
	OptionsButton:Show();
end
...
-- ============================================================
--  AutoLogin :: Character Order System
-- ============================================================

AL_realm_chars     = {}
AL_char_hooks_done = false

local AL_d2r = {}   -- display index → real game index
local AL_r2d = {}   -- real game index → display index

local function AL_BuildMaps()
	AL_d2r = {}
	AL_r2d = {}
	for displayIdx, char in ipairs(AL_realm_chars) do
		AL_d2r[displayIdx] = char.id
		AL_r2d[char.id]    = displayIdx
	end
end

-- Nomes de FontString confirmados no CharacterSelect.lua do cliente.
local function AL_RefreshButton(displayIdx)
	local char = AL_realm_chars[displayIdx]
	if not char then return end
	local p    = "CharSelectCharacterButton" .. displayIdx
	local nameFS = getglobal(p .. "ButtonTextName")
	local infoFS = getglobal(p .. "ButtonTextInfo")
	local zoneFS = getglobal(p .. "ButtonTextLocation")
	if nameFS then nameFS:SetText(char.name) end
	if infoFS then
		if char.ghost then
			infoFS:SetFormattedText(CHARACTER_SELECT_INFO_GHOST, char.level, char.class)
		else
			infoFS:SetFormattedText(CHARACTER_SELECT_INFO, char.level, char.class)
		end
	end
	if zoneFS then zoneFS:SetText(char.zone) end
end

local function AL_RefreshAllButtons()
	for i = 1, table.getn(AL_realm_chars) do
		AL_RefreshButton(i)
	end
end

-- Chamada tanto no evento do frame (fallback) quanto dentro do hook de
-- UpdateCharacterList. Guards garantem segurança nos dois cenários:
--   • Frame dispara ANTES do CharacterSelect:
--       GetNumCharacters() pode ser 0 → retorna cedo (no-op).
--       Depois CharacterSelect dispara → UpdateCharacterList hookado →
--       orig() roda, então AL_OnCharactersLoad() roda com dados prontos.
--   • CharacterSelect dispara ANTES do frame:
--       UpdateCharacterList já rodou (sem hook). Frame dispara →
--       AL_InstallCharSelectHooks() instala hooks para chamadas futuras →
--       AL_OnCharactersLoad() roda com dados já disponíveis.
function AL_OnCharactersLoad()
	local numChars = GetNumCharacters()
	if numChars == 0 then return end

	AL_realm_chars = {}
	for c = 1, numChars do
		local name, race, class, level, zone, raceFile, classFile, gender, ghost =
			GetCharacterInfo(c)
		if name then
			AL_realm_chars[c] = {
				id    = c,
	...
	-- Aplica AUTOLOGIN_CHAR_ORDER
	if AUTOLOGIN_CHAR_ORDER and table.getn(AUTOLOGIN_CHAR_ORDER) > 0 then
		local byName = {}
		for _, char in ipairs(AL_realm_chars) do byName[char.name] = char end
		local sorted, used = {}, {}
		for _, name in ipairs(AUTOLOGIN_CHAR_ORDER) do
			local char = byName[name]
			if char and not used[char.id] then
				table.insert(sorted, char)
				used[char.id] = true
			end
		end
		for _, char in ipairs(AL_realm_chars) do
			if not used[char.id] then table.insert(sorted, char) end
		end
		AL_realm_chars = sorted
	end

	AL_BuildMaps()
	AL_RefreshAllButtons()

	-- Seleciona o primeiro personagem da ordem preferida.
	-- createIndex é 0 quando não setado (sem risco de ir para criação)
	-- ou é numChars+1 (nunca igual ao id de um personagem existente).
	if AL_realm_chars[1] then
		local firstId = AL_realm_chars[1].id
		if firstId ~= CharacterSelect.createIndex then
			CharacterSelect_SelectCharacter(firstId)
		end
	end
end

function AL_InstallCharSelectHooks()
	if AL_char_hooks_done then return end

	-- UpdateCharacterList popula os botões na ordem original do servidor.
	-- Hookamos para reaplicar nossa ordem DEPOIS que ele termina.
	-- AL_OnCharactersLoad roda aqui, não no evento do frame — garantindo
	-- que GetNumCharacters() e createIndex já estão prontos.
	if UpdateCharacterList then
		local orig = UpdateCharacterList
	...
	-- UpdateCharacterSelection controla o highlight do botão selecionado.
	-- Precisa usar o índice de display, não o real game index.
	if UpdateCharacterSelection then
		local orig = UpdateCharacterSelection
		UpdateCharacterSelection = function(self, ...)
			for i = 1, MAX_CHARACTERS_DISPLAYED do
				local btn = getglobal("CharSelectCharacterButton" .. i)
				if btn then btn:UnlockHighlight() end
			end
			local displayIdx = AL_r2d[self.selectedIndex]
			if displayIdx and displayIdx > 0 and displayIdx <= MAX_CHARACTERS_DISPLAYED then
				local btn = getglobal("CharSelectCharacterButton" .. displayIdx)
				if btn then btn:LockHighlight() end
			end
		end
	end

	-- Clique simples: display index → real game index
	if CharacterSelectButton_OnClick then
		local orig = CharacterSelectButton_OnClick
		CharacterSelectButton_OnClick = function(self, ...)
			local realId = AL_d2r[self:GetID()]
			if realId then
				if realId ~= CharacterSelect.selectedIndex then
					CharacterSelect_SelectCharacter(realId)
				end
				return
			end
			orig(self, ...)
	...
	-- Clique duplo: display index → real game index + entra no mundo
	if CharacterSelectButton_OnDoubleClick then
		local orig = CharacterSelectButton_OnDoubleClick
		CharacterSelectButton_OnDoubleClick = function(self, ...)
			local realId = AL_d2r[self:GetID()]
			if realId then
				if realId ~= CharacterSelect.selectedIndex then
					CharacterSelect_SelectCharacter(realId)
